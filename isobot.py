...
class iSobot:
	...
	# t
	# -
	# that's all robots!" raise left 
	# arm, stand on left foot
	# CMD_41A = 0x4e  # "Domo arigato from isobot
	# -
	# o"
	# CMD_42A = 0x4f
	# CMD_43A = 0x50
	...
	# CMD_114B = 0x6f # "yippe yi
	# ppe" 3 times, goal post arms
	# CMD_121B = 0x70 # "ho ho ho ... <something
	# -
	# something> 
	# isobot"
	...
	# "wow that's amazing"
	# CM
	# D_131B = 0x74 # bow, arms over head and down
	# CMD_132B = 0x75
	# CMD_133B = 0x76
	# CMD_134B = 0x77
	# CMD_141A = 0x78
	...
	# # 0xB7
	# -
	# = 0xC4 single steps in different directions
	# CMD_HEADSMASH = 0xC5
	# CMD_HEADHIT = 0xC6
	# # 0xCC
	# -
	...
	# -
	# unknown (use param?
	# # after exercising one of these I am ge
	# tting only beeps 
	...
	# CMD_SQUAT = 0xDB # also = 0xDC  # doesn't work (b
	# oth)
	...
	# Initialize class
	#
	def __init__(self, port='COM8', baud=38400, databit=8, parity=None):
		print "Initializing iSobot!"
		self._port = port 	#port='/dev/tty.usbserial-	A8008pQc' # Mac default USB  #port='/dev/tty.usbserial 	-A9007KX5' # The other Mac 	USB port
		self._serialPort = serial.Serial(port, baud, bytesize=databit, parity='N') 
		# try:
		# 	self._serialPort = serial.Serial(port, baud, bytesize=databit, parity='N') #UNCOMMENT	TO RUN #UNCOMMENT 	TO RUN 
		# 	self._serialPort.open()
		# 	if self._serialPort.isOpen():
		# 		print "Serial port is opened."
		# except Exception as e:
		# 	# Catch exception in case serial connection fails
	...
	def sendCmd(self, cmd):
		...
		for f in self._serialPort.read():
			line.append(f)
			if f == '\n':
				print "Line: %s" % line
				line = []
	...
	def formatCmd(self, cmd):
		...
		return list(c)  # Must add '\r' at the end of each string
	...
	def connect(self, port, baud=38400, databit=8, par='N'):
		if port == '':
			print "No port supplied. Will use previously used port."
			port = self._port
		try:    
